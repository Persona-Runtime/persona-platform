...
# 롤백 검사용 실행기. 이 함수의 책임은 하나다 — **시도가 실제로 끝까지 갔는지**를 가른다.
# 여기서 판정하지 않는 것을 분명히 해 둔다.
#
#   이 함수가 하는 일   문장이 실행됐거나, 예상한 권한 거부로 끝났으면 0을 돌려준다.
#                       그 밖의 실패(문법 오류, 접속 실패, 컨테이너 미기동)면 1이다.
#   0이 뜻하지 않는 것  "권한이 정상"이라는 뜻이 아니다. 권한이 과도해서 문장이 성공한 경우도
#                       0이다. 두 경우를 여기서 구분하지 않는다.
#   권한 적절성 판정    6단계의 expect_denied와 7-b의 카탈로그 조회가 한다.
#   rollback 판정       호출부의 revision·잔여 테이블 비교가 한다.
#
# 실행 여부를 따로 가르는 이유는, 시도가 아예 실행되지 않았는데 "DB가 그대로"라는 이유로
# 통과하면 rollback 검사가 아무것도 확인하지 못한 채 ok를 찍기 때문이다.
